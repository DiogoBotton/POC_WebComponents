...
import { type IPortfolioProps, Portfolio } from "./Portfolio"

export const normalizeAttribute = (attribute: string) => {
  return attribute.replace(/-([a-z])/g, (_, letter) => letter.toUpperCase());
};

class PortfolioWebComponent extends HTMLElement {
    constructor() {
        super();
    ...
    connectedCallback(){
        const props = this.getPropsFromAttributes<IPortfolioProps>();
        const root = ReactDOM.createRoot(this.shadowRoot as ShadowRoot);
        root.render(<Portfolio {...props} />)
    }
